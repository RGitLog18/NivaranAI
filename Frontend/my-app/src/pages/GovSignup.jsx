...
export default function GovSignup() {
  ...
  const checkResumption = async () => {
    ...
    try {
      const res = await axios.get(`http://${import.meta.env.VITE_API_URL}:8000/api/onboarding/status?email=${form.email.trim()}`);
  ...
  const handleRequestOtp = async (e) => {
    ...
    setError('');
    try {
      await axios.post(`http://${import.meta.env.VITE_API_URL}:8000/api/gov/send-otp`, {
      ...
      await axios.patch(`http://${import.meta.env.VITE_API_URL}:8000/api/onboarding/update-step`, fd);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setError('');
    try {
      const response = await axios.post(`http://${import.meta.env.VITE_API_URL}:8000/api/gov/verify-otp`, {
